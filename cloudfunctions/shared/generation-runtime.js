...
function getSingleThemeSkeletonMap() {
  return {
    形状: [
      '找“笑脸”：找一个天然形成的、长得像人脸或动物表情的地方',
      '顺着一条线走：找一条清晰的线（比如地上的裂缝、墙上的水管、两块砖的接缝），目光或脚步跟着它，看它最终在哪消失',
      '找字母：找一个天然形成的英文字母（比如X、Y、A、O）',
      '选定一个收集：找一个特定的形状（圆形/正方形/三角形/星形/……），在本次漫步中收集5个',
      '踩影子：找一个投射形状奇特又有趣的影子，试着踩在它的边缘上',
      '找个洞看：找一个天然的“孔隙”或“框架”（墙洞、树杈间隙、铁网缝），凑近透过它看背后是什么',
    ],
    色彩: [
      '带着自己的颜色走：选一个自己身上的颜色，在环境里“捕获”跟它一样的颜色',
      '找“不属于这里”的色：找一个极其扎眼、格格不入的小色块',
      '选定一个收集：找一个特定的颜色（红色/蓝色/绿色/黄色/……），在本次漫步中收集5个',
      '找褪色的痕迹：找一处被时间“改变”了的颜色，比如被太阳晒得泛白的旧海报、斑驳掉漆的铁门，或者被雨水冲刷出的水渍色',
      '闭眼转身盲测：闭上眼睛停三秒，原地随意转个方向再突然睁开眼，记住第一秒钟直接“撞”进你眼睛里的那块颜色是什么',
      '眯起眼睛看：停下来，试着把眼睛眯成一条缝，让眼前的风景变模糊，看看细节消失后，这个空间剩下了哪两三种最主要的“大色块”',
    ],
    声音: [
    ...
    数字: [
      ...
      '选定一个收集：找一个特定的数字，在本次漫步中收集5个',
      '数一数：视野中同类物体有几个',
      '数字指令：数字作为行动指令，比如看见的数字决定步数',
    ],
    气味: [
