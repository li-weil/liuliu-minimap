...
- `2026-04-13-ai-validation-primary-r1`
...
- 模型现在只看到精简后的 `targetThemes / time / nearby / sceneCards / themeReferences`
...
- `ragModelInput.sceneCards[].missionHints` 会按已选主题过滤。
...
当前 `ragModelInput` 已进一步瘦身，单主题和组合主题都会尽量收敛为这几类信息：
...
也就是说，当前真正提交给模型的不是完整 `ragContext`，而是从 `ragContext` 中压出来的一份最小必要入模对象。
...
- 进阶模式保留 3 条任务
- 去掉相似任务
- 任务不足时优先补 fallback mission，不够再补 anchored mission
- 返回 `finalization` 调试信息，说明是否被后处理改写
...
如果看到 `rag+ai`，说明不是 fallback；在当前版本下，这类结果默认也已经过 AI 复核。
...
- 是否包含样例原句
- `targetThemes` 是否正确
- `time` 是否只保留当前时间段真正有用的信息
- `nearby` 是否只保留 POI、主场景和活动线索
- `sceneCards` 是否仍混入错误场景
- `themeReferences` 是否只保留本主题真正需要的 angle 和 cues

单主题额外建议看：

- `themeReferences[].angle`
  - 是否是“轮廓节奏 / 天气显色 / 回响层次 / 数字变体 / 来源判断”这类可读角度。
- `sceneCards[].missionHints`
  - 是否已经是单主题过滤后的结果。
...
如果 `source=rag+ai`，但你感觉句子还是很模板化，不代表“没做 AI 复核”。

当前更常见的原因是：

- AI 生成成功，所以不是 fallback
- `finalizeTheme()` 做了后处理改写
- RAG 传给模型的角度仍然太集中

在当前版本下，AI 复核是每次都跑的，不再存在“规则先通过，所以完全没走 AI 复核”的情况。
