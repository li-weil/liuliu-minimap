...
1. 用户在探索页选择生成方式。
2. 前端确认探索点、时间、天气、模式、主题、偏好，以及附近 AOI、商圈、POI 等地点上下文。
...
- 调试面板中的完整模型输入、模型输出、结构检查、关键上下文和地点原始信息。
...
地点信息主要来自 `miniprogram/utils/amap.js#getRegeo()` 的高德 REST 逆地理结果，不依赖普通周边 POI 检索。

`getRegeo()` 是项目自己的统一封装。它会直接请求高德 REST 逆地理接口，手动归一化为接近小程序 SDK 的结构，并把原始地点语境保留在 `nativeContext` 中。接口缺少 `roads`、`pois`、`aois` 时不做 mock，空值保持为空。
...
- `nearby.aoi`、`nearby.aoiList` 可能为空，这通常表示高德没有返回 AOI，不等同于逆地理失败。
- `nearby.poiNames`、`representativePoiNames` 主要用于调试和 fallback，不作为模型的核心约束。
- 现在的生成更依赖主题、时间、天气、偏好、任务骨架、省市区和可用的 AOI / 商圈语境，避免被具体点位名带偏。
...
- 如果 AOI 为空，模型仍会继续使用地区、时间、天气、主题骨架和偏好信息生成任务。
- 模型主要依据中文文本理解语境。
...
AOI 是高德逆地理的可选结果，不保证每个坐标都有。海岛、村镇、普通街巷或部分公共设施周边可能返回正常地址和 POI，但 `aois` 为空。当前前端不会 mock AOI，也不会把空 AOI 当成生成失败。
...
这些信息仍可以在调试面板中查看，用来判断定位和地图上下文是否合理。如果 AOI 为空，要结合“地点原始信息”里的 `nativeContext.pois`、`addressComponent`、`reason` 判断是接口失败，还是高德确实没有返回 AOI。
...
- 高德逆地理和地点语境实际返回了哪些 POI、AOI、商圈、道路、地址组件。
- 模型实际看到了什么。
- 模型实际返回了什么。

“地点原始信息”来自 `buildLocationRawDebugSnapshot()`，会展示：

- `location`：探索点经纬度、名称和地址。
- `nearby`：前端整理后的附近摘要，包括 `aoi`、`aoiList`、`businessAreas`、`poiNames`。
- `locationContextResponse`：地点语境结果、`primaryAoiName`、`primaryAoiTypecode` 和 `reason`。
- `nativeContext`：原始地址组件、主 AOI、AOI 列表、商圈、POI 列表和道路列表。

如果 `aoi` / `aoiList` 为空，但 `nativeContext.pois` 和 `addressComponent` 有内容，通常说明高德对该坐标没有返回 AOI，而不是网络失败。
...
这套流程的核心取向是：减少具体地点名对模型的过度绑架，把生成自由度还给模型，同时通过省市区、可用的 AOI / 商圈语境、时间、主题骨架、偏好 instruction 和轻量规则维持基本质量。
...
3. 完善不同时间和天气组合下的时间骨架。
4. 继续人工打磨各主题的任务骨架。
5. 继续观察 AOI 质量；对 AOI 为空但 POI 有内容的坐标，优先通过“地点原始信息”判断能否补充更稳定的场域字段，不直接强塞进主 prompt。
6. 对前端等待体验做更细腻的进度反馈。
