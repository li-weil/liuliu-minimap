...
6. shared runtime 压标题、压描述、压任务长度，并兜底补足“附近锚点”
7. 主题卡片展示结果
...
其中 `ragModelInput` 已移除知识库样例原句，只把角度、线索、锚点、反例和检索分数传给模型，降低模型照着 sample 改写的概率。
...
- 限制任务长度
- 去掉相似任务
- 若任务没锚到“附近”或“时间”，自动补一条 anchored mission

也就是说，最终展示给用户的任务，不完全等于模型原始回包，而是“模型回包 + 共享运行时整理”的结果。

### 8.5 统一验证机制

当前纯粹模式、进阶模式、组合模式都复用 `summarizeThemeValidation()` 做规则校验。
...
AI 二次复核不是每次都跑。只有当规则校验发现主题、在地、泛化、重复或低分问题时，云函数才会按需调用二次复核，并可能局部重写后再次收口。

最新补充：

- 校验会拦“形状写成数量统计”“色彩写成听声”“气味写成数数”这类明显跑偏。
- 校验也会放过“数字形状”“光影轮廓”这类服务主主题的合理借词，避免误杀。
